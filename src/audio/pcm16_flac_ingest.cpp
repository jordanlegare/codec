...
Result<void> validate_request(const Pcm16FlacIngestRequest& request) {
  if (request.source_uri.empty()) {
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest requires a source URI");
  }
  if (request.archive_path.empty() || request.archive_path.filename().empty()) {
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest requires an archive file path");
  }
  if (request.end_ns < request.start_ns) {
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest interval is inverted");
  }
  if (request.capture_chunk_bytes < minimum_capture_chunk_bytes ||
      request.capture_chunk_bytes > maximum_capture_chunk_bytes) {
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest capture chunk is outside the supported bounds");
  }
  if (request.maximum_source_bytes == 0 ||
    ...
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest source limit is outside the process bounds");
  }
  if (request.maximum_decoded_pcm_bytes == 0 ||
      request.maximum_decoded_pcm_bytes >
          static_cast<std::uint64_t>(
              std::numeric_limits<std::size_t>::max())) {
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest decoded PCM limit is outside the process bounds");
  }
  if (request.maximum_redirects > maximum_capture_redirects) {
    return fail(ErrorCode::invalid_argument,
                "FLAC audio ingest redirect limit is outside the supported bounds");
  }
  if (request.descriptor.type != StreamType::audio ||
      request.descriptor.payload_type != "audio/flac") {
    return fail(ErrorCode::invalid_argument,
  ...
  if (inspection_error == std::errc::no_such_file_or_directory) {
    ...
    return fail(ErrorCode::archive_io,
                "cannot inspect FLAC audio ingest archive output: " +
  ...
  if (!inspection_error &&
    ...
    return fail(ErrorCode::archive_io,
                "FLAC audio ingest refuses to replace an existing archive path");
  }
  return {};
}
...
Result<Pcm16FlacIngestReport> ingest_pcm16_flac(
      ...
      [&source_bytes, maximum_source_bytes = request.maximum_source_bytes](
        ...
        if (current_size > maximum_source_bytes ||
          ...
          return fail(ErrorCode::resource_exhausted,
                      "FLAC audio ingest source exceeds the configured limit");
        }
  ...
  const auto finish_source_only =
      [&writer, &report](Error error) -> Result<Pcm16FlacIngestReport> {
  ...
  auto state = detail::decode_flac_pcm16(source_bytes,
                                         request.maximum_decoded_pcm_bytes);
