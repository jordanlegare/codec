...
old = "project(CODEC VERSION 0.2.0 LANGUAGES C CXX)"
new = "project(CODEC VERSION 0.3.0 LANGUAGES C CXX)"
if old not in text and new not in text:
    raise SystemExit("unexpected CMake project version")
...
new = "  version: 0.3.0"
if old not in text and new not in text:
    raise SystemExit("unexpected README manifest version")
readme.write_text(text.replace(old, new, 1))

changelog = Path("CHANGELOG.md")
text = changelog.read_text()
marker = "## Unreleased\n\n"
release = "## 0.3.0 — 2026-08-29\n\n"
...
worksheet = Path("AI_WORKSHEET.md")
text = worksheet.read_text()
start = text.find("## Active work record —")
end = text.find("## 0. Work record")
if start < 0 or end < 0 or end <= start:
    raise SystemExit("cannot locate active worksheet record")
active = """## Active work record — Release v0.3.0
...
"""
worksheet.write_text(text[:start] + active + text[end:])
