...
Add an explicit compatibility-preserving C++ Audio Stream Profile facade for the already-implemented WAV/PCM, W0 signed-statement, W1/W2 watermark, and source-separation APIs.
...
- Existing `codec::WavPcm16`, signed-statement, watermark, separation APIs, legacy headers, CLI, and C ABI remain unchanged.
- No CODA/archive format or truth-class behavior changes.

## TDD proof

1. Add a test that includes `codec/profiles/audio.hpp`, proves profile/root type identity, and invokes the existing statement, watermark-name, and default separation backend behavior through the profile namespace.
