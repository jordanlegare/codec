...
5. Run fresh ASan/UBSan CTest and direct tests on the same head. Disable local
   leak scanning only if the managed `/proc` restriction reproduces; require
   normal leak-enabled remote sanitizer CI.
...
2. Publish the exact fully verified green tree without force, open one D.9 PR,
   and require GCC, Clang, package/install, and leak-enabled sanitizer success
   on that exact head.
